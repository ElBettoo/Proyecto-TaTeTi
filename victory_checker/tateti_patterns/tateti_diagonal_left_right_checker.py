...
class TatetiDiagonalRightToLeftChecker(TatetiVictoryPattern):
    ...
    def get_possible_cords_by_last_move(self, cantidad_casilleros: tuple[int, int], last_move: tuple[int, int], win_length) -> list[Casillero]:
        ...
        distancia_minima = min(((cantidad_columnas - 1) - move_pos_x), move_pos_y, win_length-1) # Valor minimo entre los casilleros para salirse del tablero en 'x' e 'y' y ademas la distancia para ganar necesaria
        ...
        while (start_pos_x >= 0 and start_pos_y <= cantidad_filas-1) and (casilleros_despues_de_last_move < win_length): # 2 = max_x - 1       (-1) porque el indice arranca de 0   (0) porque es la primer columna (limite)
